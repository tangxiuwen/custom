...
class b2c_order_total
{
    ...
    public function payment_detail(&$controller, $cart, $sdf_order)
    {
        ...
        if ($sdf_order['is_rx'] === true) {
            ...
            $sdf_order['shipping_id'] = $dly_info['dt_id'];
            // 设置默认 支付方式
            $sdf_order['payment'] = -1;
        }
        ...
        //红包使用
        if ($sdf_order['use_redpacket'] == 'true') {
            $share_data = app::get('weixin')->model('share')->getRow('*', array('share_id' => $sdf_order['share_id']));
            $radio = $objMath->number_div(array($share_data['deduction_limit'], 100));
            $deduction_limit = $objMath->number_multiple(array($radio, $cost_item));
            if ($sdf_order['redpacket_money'] > $deduction_limit) {
                $sdf_order['redpacket_money'] = $deduction_limit;
            }
            $total_amount = $objMath->number_minus(array($total_amount, $sdf_order['redpacket_money']));
            if ($total_amount < 0) {
                $redpacket_money = $total_amount;
            } else {
                $redpacket_money = $sdf_order['redpacket_money'];
            }
        } else {
            $redpacket_money = 0;
        }
        ...
        $payment_detail = array('cost_item' => $objCurrency->amount_nocur($cost_item, $sdf_order['cur'], false, false),
                                ...
                                'redpacket_pay' => $redpacket_money,
                                'share_id' => $sdf_order['share_id'],
        );
