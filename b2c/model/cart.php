...
class b2c_mdl_cart extends dbeav_model
{
     ...
     */
    public function count_objects(&$aData, $is_rx = 'false')
    {
        ...
        foreach (kernel::servicelist('b2c_cart_object_apps') as $object) {
            if (!is_object($object)) continue;
            $aResult = $object->count($aData, $is_rx);
     ...
     * @return boolean
     */
    public function is_empty($aCart, $is_empty = false)
    {
        ...
        $mCartObject->remove_object('', null, $msg, false, $is_empty);
        return true;
    }
