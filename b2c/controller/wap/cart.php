...
class b2c_ctl_wap_cart extends wap_frontpage{
       ...
       /* if ($type != 'coupon') {
        ...
        if(!$obj_ident){
            ...
            if(isset($aData['btype']) && $aData['btype'] == 'is_fastbuy') {
                //判断是否是处方药
                $this->check_rx($data['goods']['goods_id']);
    ...
    public function _check_checkout($isfastbuy=0){
        ...
        //判断是否是处方药
        if($this->is_rx){
            $this->splash('success',$this->gen_url(array('app'=>'b2c','ctl'=>'wap_cart','act'=>'checkout','arg0'=>$isfastbuy,'arg1'=>1)),'去确认...','','',true);
        }else{
    ...
    public function checkout($isfastbuy=0,$is_rx=0)
    {
        ...
        $this->checkout_result($isfastbuy,$is_rx);
    }
     ...
     * @return bool
     * */
    public function check_rx($goods_id)
    {
        $is_rx = $this->app->model('goods')->getRow('prescription', array('goods_id' => $goods_id));
        if ($is_rx['prescription'] == '2') {
            $res = true;
            $this->is_rx = true;
        } else {
            $res = false;
     ...
     * @return null
     */
    public function checkout_result($isfastbuy=0,$is_rx=0){
        //预售商品下单结算的判断
        if(!$isfastbuy)
        {
            $prepare=kernel::service('prepare_goods');
        ...
        if (strpos($_SERVER['HTTP_USER_AGENT'], 'MicroMessenger') !== false) {
            $obj_share = app::get('weixin')->model('share');
            $obj_draw = app::get('weixin')->model('draw');
            $_filter = array(
                'member_id' => $arrMember['member_id'],
                'is_use' => '0',
            );
            $redpacket = $obj_draw->getList('*', $_filter, 0, -1, 'createtime asc');
            foreach ($redpacket as $k => $v) {
                $activity = $obj_share->getRow('*', array('share_id' => $v['share_id']));
                if ($v['createtime'] < strtotime(date("Y-m-d H:i:s", time() - 86400 * $activity['validity']))) {
                    unset($redpacket[$k]);
                    continue;
                }
                if ($total_item < $activity['conditions']) {
                    unset($redpacket[$k]);
                    continue;
                }
            }
            if (!empty($redpacket)) {
                $redpacket = current($redpacket);
                $this->pagedata['has_redpacket'] = true;
                $this->pagedata['share_id'] = $redpacket['share_id'];
                $this->pagedata['draw_id'] = $redpacket['draw_id'];
                $this->pagedata['member_redpacket'] = $redpacket['get_money'];
            } else {
                $this->pagedata['has_redpacket'] = false;
            }
        } else {
            $this->pagedata['has_redpacket'] = false;
        }
    ...
    public function total(){
        ...
        $sdf_order['use_redpacket'] = $_POST['use_redpacket'];
        $sdf_order['redpacket_money'] = $_POST['redpacket_money'];
        $sdf_order['share_id'] = $_POST['share_id'];
