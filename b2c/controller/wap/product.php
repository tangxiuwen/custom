...
class b2c_ctl_wap_product extends wap_frontpage{
    ...
    public function index() {
        ...
        $productId = $productsModel->getRow('product_id', array('bn' => $productId));
        if (empty($productId)) {
...
#        $this->pagedata['goods_share'] = $goodsshare;
        ...
        //如果是处方药则只显示立即购买 by hdz
        if($aGoods['prescription']=='2'){
            $this->pagedata['is_rx']=true;
            $btn= $this->_get_servicelist_by('b2c_products_index_btn');
            unset($btn['80']);
        }else{
            $btn= $this->_get_servicelist_by('b2c_products_index_btn');
        }
        $this->pagedata['btn_page_list'] = $btn;
