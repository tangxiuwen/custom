        ...
        $arr_bases = array(
            ...
            array(
                ...
                'items'=>array(
                    ...
                    array('label'=>app::get('b2c')->_('红包活动'),'app'=>'b2c','ctl'=>'site_member','link'=>'draw'),
                    $arr_blance,//预存款充值
...
//        exit;
                    ...
                    if ($arr_objects['obj_type'] == 'goods')
                    {
                        foreach ($arr_objects['order_items'] as $arr_items)
                        {
                            ...
                            if ($arr_items['item_type'] == 'product')
                            {
                                ...
                                $arr_data_item['goods_items'][$k]['product']['link_url'] = $this->gen_url(array('app'=>'b2c','ctl'=>'site_product','act'=>'index','arg0'=>$arr_items['products']['bn']));;
                                if ($arr_items['addon'])
                                {
    ...
    function deposit(){
        ...
        $payment_info = $mdl_payment_cfgs->getPaymentInfo('deposit');
        if ($payment_info['app_staus'] != app::get('ectools')->_('开启')) {
            header('Location: ' . $this->gen_url(array('app' => 'b2c', 'ctl' => 'site_member', 'act' => 'index')));
            exit;
        }
        ...
        foreach($aProduct as $k=>$v){
            if($v['nostore_sell']){
                ...
                foreach((array)$v['spec_desc_info'] as $value){
                    ...
                    $productId = $objProduct->getRow('bn,goods_id', array('product_id' => $value['product_id']));
                    $goodsId=$objGoods->getRow('prescription',array('goods_id'=>$productId['goods_id']));
                    $aProduct[$k]['bn'] = $productId['bn'];
                    $aProduct[$k]['spec_info'] = $value['spec_info'];
                    $aProduct[$k]['price'] = $value['price'];
                    $aProduct[$k]['is_rx'] = $goodsId['prescription'];
...
/*红包活动*/
    ...
    {
        $this->path[] = array('title' => app::get('b2c')->_('会员中心'), 'link' => $this->gen_url(array('app' => 'b2c', 'ctl' => 'site_member', 'act' => 'index', 'full' => 1)));
        $this->path[] = array('title' => app::get('b2c')->_('红包活动'), 'link' => '#');
        $GLOBALS['runtime']['path'] = $this->path;
...
//        $rows =  ceil($tota/$this->pagesize);
//        $this->pagedata['total'] = $rows;
        //$aData = $oDraw->insertinto();
//        echo '<pre>';
//        print_r($aData);
//        exit;
    ...
    public function reAddCart($order_id,$archive=false){
        //先判断是否处方订单
        $orders_mdl = $this->app->model('orders');
        $order_type = $orders_mdl->getRow('is_rx', array('order_id' => $order_id));
        if($order_type['is_rx']=='N'){
            $url = $this->gen_url(array('app'=>'b2c', 'ctl'=>'site_cart'));
        }else{
            $url = $this->gen_url(array('app'=>'b2c', 'ctl'=>'site_cart', 'act'=>'rx'));
        }
        ...
        foreach($cartData as $goods){
            //判断是否是处方药
            $goods_type=$goods_mdl->getRow('prescription',array('goods_id'=>$goods['goods']['goods_id']));
            if($goods_type['prescription']=='2'){
                $goods['goods']['is_rx']=true;
            }
    ...
    function docancel(){
        ...
        $sdf_order_member_id = $mdl_order->getRow('member_id,is_rx', array('order_id'=>$order_cancel_reason['order_id']));
        if($sdf_order_member_id['is_rx']=='Y'){
            $notice_arr=array(
                '请勿取消别人的需求',
                '需求取消原因记录失败',
                '需求取消成功',
        ...
        {
            $db->rollback();
            $this->splash('error','',$notice_arr[0],true);
            return;
        }
        ...
        {
            $db->rollback();
            $this->splash('error','',$notice_arr[1],true);
        }
        ...
        if ($b2c_order_cancel->generate($sdf, $this, $message))
        {
            ...
            $this->splash('success',$url,$notice_arr[2],true);
        }
        else
        {
            $db->rollback();
            $this->splash('error','',$notice_arr[3],true);
        }
    }
}
