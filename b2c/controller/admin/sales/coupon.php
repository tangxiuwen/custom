...
class b2c_ctl_admin_sales_coupon extends desktop_controller{
    ...
    function _prepareData($aData) {
        ...
        //区域处理-查询所选地区的所有三级地区-用于优惠规则匹配
        $regionidAll = '';
        if ($aResult['rule']['area']['areaGroupId']) {
            $areas = explode(',', $aResult['rule']['area']['areaGroupId']);
            foreach ($areas as &$strArea) {
                if (strpos($strArea, '|') !== false) {
                    ...
                    $regionpath = $objRegions->getRow('region_path', array('region_id' => $strArea));
                    $regionidarr = $objRegions->getList('region_id', array('region_path|head' => $regionpath['region_path']));

                    foreach ($regionidarr as $_item) {
                        $regionidAll .= $_item['region_id'] . ',';
                    }
                } else {
                    $regionidAll .= $strArea . ',';
                }
        ...
        $aResult['rule']['area_str'] = $regionidAll ? $regionidAll : '0';
