...
final class ectools_payment_plugin_wapupacp extends ectools_payment_app implements ectools_interface_payment_app {
    ...
    public function __construct($app){
		...
		$this->notify_url = kernel::openapi_url('openapi.ectools_payment/parse/wap/ectools_payment_plugin_wapupacp', 'callback');
    ...
    public function callback(&$recv)
	{
        ...
        $this->sdk_verify_cert_dir = DATA_DIR . '/cert/payment_plugin_wapupacp/';
        $this->sdk_sign_cert_path = $this->sdk_verify_cert_dir.trim($this->getConf('sdk_sign_cert_path', __CLASS__));
        error_log(date('Y-m-d H:i:s') . '=' . var_export($recv, 1), 3, DATA_DIR . '/recv1.txt');
        if($this->is_return_vaild($recv) === 1){
            error_log(date('Y-m-d H:i:s') . '=' . var_export($recv, 1), 3, DATA_DIR . '/recv2.txt');
        ...
        error_log(date('Y-m-d H:i:s') . '=' . var_export($ret, 1), 3, DATA_DIR . '/ret.txt');
		return $ret;
    }
    ...
    public function getPulbicKeyByCertId($certId) {
        ...
        $filePath = ROOT_DIR . "/custom/ectools/lib/payment/plugin/wapupacp/" . "UpopRsaCert.cer";
