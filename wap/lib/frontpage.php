...
class wap_frontpage extends wap_controller{
    ...
    public function set_weixin_openid(){
        ...
        if( $openid ){
            ...
            if( $bindTagData ){
                ...
                $this->free_login($bindTagData['member_id']);
            }else{
    ...
    public function free_login($member_id)
    {
        $b2c_user_object = kernel::single('b2c_user_object');
        $b2c_user_object->set_member_session($member_id);
