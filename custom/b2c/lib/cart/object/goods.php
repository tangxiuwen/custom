...
class b2c_cart_object_goods implements b2c_interface_cart_object
{
    ...
    public function add_object($aSave, &$msg = '', $append = true, $is_fastbuy = false)
    {
        ...
        foreach ($this->getAll(false, $is_fastbuy, $is_rx) as $val) {
    ...
    public function update($sIdent, $quantity, $is_rx = false)
    {
    ...
    // 购物车里的所有商品项
    public function getAll($rich = false, $is_fastbuy = false, $is_rx = false)
    {
        if ($this->get_cart_status()) {
    ...
    {
        $util_mdl = kernel::single('b2c_util');
        if ($util_mdl->get_cart_type() == 'require') {
            return $this->oCartObjects->delete(array('member_ident' => $this->member_ident, 'obj_type' => 'goods', 'is_rx' => 'true'));
        } else {
            return $this->oCartObjects->delete(array('member_ident' => $this->member_ident, 'obj_type' => 'goods', 'is_rx' => 'false'));
        }
    }

    // 统计购物车中商品项数据
    public function count(&$aData, $is_rx = 'false')
    {
        // 购物车中不存在goods商品
    ...
    private function _get($aData)
    {
        ...
        foreach ($aData as $key => $row) {
            ...
            $arr_goods[$key] = $goods_id = $row['params']['goods_id'];
