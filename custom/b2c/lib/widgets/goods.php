...
class b2c_widgets_goods extends b2c_widgets_public {
    ...
    public function getGoodsList($filter,$platform='site'){
        ...
        if($gids){
            $productData = $objProduct->getList('goods_id,product_id,bn,marketable',array('goods_id'=>$gids,'is_default'=>'true'));
            foreach($productData as $k=>$val){
                $_return['goodsRows'][$val['goods_id']]['products'][] = $val;
                $result['goodsRows'][$val['goods_id']]['goodsLink'] = $this->getGoodsLink($val['bn'],$platform);
            }
        }
