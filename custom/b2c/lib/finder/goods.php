...
class b2c_finder_goods{
    ...
    function detail_basic($gid){
        ...
        $goods=$o->getList(
            ...
            rank_count'
            ,array('goods_id'=>$gid));
        $default_product = app::get('b2c')->model('products')->getList('product_id,bn',array('goods_id'=>$gid,'is_default'=>'true'));
        if(!$default_product){
            $default_product = app::get('b2c')->model('products')->getList('product_id,bn',array('goods_id'=>$gid));
        }
        $goods = current($goods);
        $render->pagedata['goods'] = &$goods;
        $render->pagedata['is_pub'] = ($goods['marketable']!='false');
        $render->pagedata['url'] = app::get('site')->router()->gen_url(array('app'=>'b2c','ctl'=>'site_product','full'=>1,'act'=>'index','arg'=>$default_product[0]['bn']));
