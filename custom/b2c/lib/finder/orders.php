...
class b2c_finder_orders{
    ...
    //    $_is_all_return_ship = 1;
        ...
        $orderinfo = $objDraw->getRow('*', array('order_id' => $order_id));
        $share_name = $objShear->getRow('name', array('share_id' => $orderinfo['share_id']));
        $render->pagedata['order']['share_name'] = $share_name['name'];
        $render->pagedata['order']['practical_money'] = $orderinfo['practical_money'];
    ...
    public function detail_draw($order_id){
        $render = $this->app->render();
        $objDraw = app::get('weixin')->model('draw');
        $objShear = app::get('weixin')->model('share');
        $objOrders = app::get('b2c')->model('orders');
        $orderinfo = $objDraw->getRow('*', array('order_id' => $order_id));
        $discount = $objOrders->getRow('*', array('order_id' => $order_id));
        $share_name = $objShear->getRow('name', array('share_id' => $orderinfo['share_id']));
        $orderinfo['share_name'] = $share_name['name'];
        $orderinfo['redpacket_pay'] = $discount['redpacket_pay'];
        $render->pagedata['drawlist'] = $orderinfo;
        return $render->fetch('admin/order/od_draw.html',$this->app->app_id);
    }
    ...
    function column_check($row)
    {

        $view=$_GET['view']?:0;
        $page=$_GET['page']?:1;
        $orders = $this->app->model('orders');
        $res = $orders->getList('*', array('order_id' => $row['order_id']));
        if ($res[0]['is_rx'] == 'Y'  && $res[0]['disabled'] == 'false' && $res[0]['status']!='dead' && $res[0]['status']!='finish') {
            if ($res[0]['is_pass'] == 'I') {
                $row = "<a href='index.php?app=b2c&ctl=admin_order&act=docheck&status=1&id={$res[0]['order_id']}&view={$view}&page={$page}' class='pointer'><span>通过</span></a>&nbsp;<a href='index.php?app=b2c&ctl=admin_order&act=docheck&status=0&id={$res[0]['order_id']}&view={$view}&page={$page}' class='pointer'><span>拒绝</span></a>";
            }else{
                $row = "";
            }
        } else {
