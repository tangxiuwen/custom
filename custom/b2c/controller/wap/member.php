...
#        $this->pagedata['orders'] = $aData['data'];
        ...
        foreach($aData_fav['data'] as $k => $v){
            $productBn = $productsModel->getRow('bn', array('product_id' => $v['spec_desc_info'][0]['product_id']));
            $aData_fav['data'][$k]['spec_desc_info'][0]['bn'] = $productBn['bn'];
        }
                    ...
                    if ($arr_objects['obj_type'] == 'goods')
                    {
                        foreach ($arr_objects['order_items'] as $arr_items)
                        {
                            ...
                            if ($arr_items['item_type'] == 'product')
                            {
                                ...
                                $arr_data_item['goods_items'][$k]['product']['link_url'] = $this->gen_url(array('app'=>'b2c','ctl'=>'wap_product','act'=>'index','arg0'=>$arr_items['products']['bn']));;
                                if ($arr_items['addon'])
                                {
...
// echo "<pre>";print_r($this->pagedata['order']);exit;
        ...
        $deli = $objDelivery->getRow('logi_id,logi_no', array('order_id' => $sdf_order['order_id']));
        if ($deli['logi_no']) {
            $this->pagedata['gourl'] = 1;
        }
        /*$objDelivery = $this->app->model('delivery');
        $objDlycorp  = $this->app->model('dlycorp');
        $deli = $objDelivery->getRow('logi_id,logi_no',array('order_id' => $sdf_order['order_id']));
        $callbackurl = $this->gen_url(array('app' => 'b2c', 'ctl' => 'wap_member', 'act' => 'orderdetail','full'=>1 ,'arg0' => $sdf_order['order_id']));
        $baseurl = 'http://m.kuaidi100.com/index_all.html?';
        if(!empty($deli)){
            $bn = $objDlycorp->getRow('corp_code',array('corp_id' => $deli['logi_id']));
            $gourl = $baseurl . 'type=' . $bn['corp_code'] . '&postid=' . $deli['logi_no'] . '&callbackurl=' . $callbackurl;
            $this->pagedata['gourl'] = $gourl;
        }*/
    ...
    function deposit(){
        ...
        if ($payment_info['app_staus'] != app::get('ectools')->_('开启')) {
            header('Location: ' . $this->gen_url(array('app' => 'b2c', 'ctl' => 'wap_member', 'act' => 'index')));
            exit;
        }
    ...
    function favorite($nPage=1){
        ...
        foreach($aProduct as $k=>$v){
            if($v['nostore_sell']){
                ...
                $productId = $objProduct->getRow('bn', array('product_id' => $v['spec_desc_info'][0]['product_id']));
                $aProduct[$k]['bn'] = $productId['bn'];
            }else{
                foreach((array)$v['spec_desc_info'] as $value){
                    $aProduct[$k]['product_id'] = $value['product_id'];
                    $productId = $objProduct->getRow('bn', array('product_id' => $value['product_id']));
                    $aProduct[$k]['bn'] = $productId['bn'];
    ...
    */
    ...
    public function nodiscuss($nPage=1){
        ...
        foreach ($sell_logs_data as $k => $v) {
            $productBn = $productsModel->getRow('bn', array('product_id' => $v['product_id']));
            $sell_logs_data[$k]['bn'] = $productBn['bn'];
            $res=$orders_mdl->getList('is_rx',array('order_id'=>$v['order_id']));
            if($res[0]['is_rx']=='Y')unset($sell_logs_data[$k]);
        }
...
//我的红包活动
    function draw($nPage=1) {
        $this->path[] = array('title'=>app::get('b2c')->_('会员中心'),'link'=>$this->gen_url(array('app'=>'b2c', 'ctl'=>'wap_member', 'act'=>'index','full'=>1)));
        $this->path[] = array('title'=>app::get('b2c')->_('我的红包'),'link'=>'#');
        $GLOBALS['runtime']['path'] = $this->path;
        $rows = app::get('weixin')->model('draw')->getList('*',array('member_id'=>$this->app->member_id));
        $oDraw = kernel::single('b2c_draw_mem');
        $aData = $oDraw->get_list_m($this->app->member_id,$nPage);
        $tota = count($rows);
        $this->pagination($nPage,ceil($tota/$this->pagesize),'draw');
        $objShear = app::get('weixin')->model('share');
        foreach ($aData as $k=>$v){
            $share = $objShear->getRow('name,validity',array('share_id'=>$v['share_id']));
            $aData[$k]['share_name'] = $share['name'];
            $aData[$k]['valid_time'] = $share['validity']*86400+$v['createtime'];
        }
        $this->pagedata['data'] = $aData;
        $this->page('wap/member/draw.html');
    }
    /**
     * 添加留言
     * @params string order_id
    ...
    public function check_express($order_id){
        $error_url = $this->gen_url(array('app' => 'b2c', 'ctl' => 'wap_member', 'act' => 'orderdetail', 'arg0' => $order_id));
        $objDelivery = $this->app->model('delivery');
        $row = $objDelivery->getRow('delivery_id',array('order_id' => $order_id));
        if(empty($row)){
            $this->splash('failed', $error_url, '仓库正在处理中', true);
        }
        logisticstrack_puller::pull_logi($row['delivery_id'], $data);
        if($data['msg'] != 'ok'){
            $this->splash('failed', $error_url, $data['msg'], true);
        }
        $this->pagedata['list'] = $data['data'];
        $this->page('wap/member/check_express.html');
    }
}
