...
class b2c_ctl_wap_order extends wap_frontpage{
    ...
    public function create()
    {
        ...
        if (!$_POST['delivery']['ship_area'] ||
            !$_POST['delivery']['ship_addr'] ||
            !$_POST['delivery']['ship_name'] ||
            (!$_POST['delivery']['ship_mobile'] && !$_POST['delivery']['ship_tel']))
        {
            if (!$_POST['delivery']['ship_area'] )
            {
        ...
        if ($_POST['is_rx']) {
            // 设置默认 配送方式
            $dly_model = app::get('b2c')->model('dlytype');
            $filter = array('has_cod' => 'true', 'dt_status' => 1, 'disabled' => 'false');
            $dly_info = $dly_model->getRow('*', $filter, 'ordernum desc');
            $_POST['delivery']['shipping_id'] = $dly_info['dt_id'];
            $_POST['payment']['pay_app_id'] = -1;
        }
        ...
        if (!empty($order_data['share_id'])) {
            $share_id = $order_data['share_id'];
            $draw_id = $order_data['draw_id'];
        }
        unset($order_data['share_id']);
        unset($order_data['draw_id']);
		...
		if ($result)
		{
			...
			if ($is_need_rpc)
			{
              /*
                ...
                if($_POST['is_rx']){
                    //存到数据库
                    $rx_info['order_info']=serialize($order_data);
                    $rx_info['order_id']=$order_data['order_id'];
                    $this->app->model('rx_order')->save($rx_info);
                }else {
        ...
        //使用红包
        if ($order_data['redpacket_pay']) {
            $objDraw  = app::get('weixin')->model('draw');
            $redpacket_use_data = array(
                'is_use' => '1',
                'order_id' => $order_data['order_id'],
                'use_time' => time(),
                'practical_money' => $order_data['redpacket_pay'],
            );
            if (!$objDraw->update($redpacket_use_data, array('draw_id' => $draw_id))) {
                $this->end(false, app::get('b2c')->_('订单生成失败 : 红包金额更新失败!'), $this->gen_url(array('app' => 'b2c', 'ctl' => 'site_cart', 'act' => 'checkout')), true, true);
            }
        }
