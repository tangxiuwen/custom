...
class b2c_ctl_site_cart extends b2c_frontpage
{
    ...
    public function add($type = 'goods')
    {
      ...
      /*  if(array_key_exists('stock',$data)){
            //只有详情页才存在stock这个key ， 其实这个就是用来判断是否是商品详情页
            $continue=true;
        }*/

        /*if ($type != 'coupon') {
            $goods_id = empty($data[1]) ? $data['goods']['goods_id'] : $data[1];

            $flag = $this->app->model('goods')->getRow('prescription', array('goods_id' => $goods_id));
                if ($flag['prescription'] == '2' && empty($continue)) {
                    $msg = app::get('b2c')->_('处方药不能加入购物车！');
                    if ($post['mini_cart']) {
        ...
        if ($aData['btype'] == 'is_fastbuy') {
            ...
            /*if($data[4]==true){
                $data['goods']['is_rx']=$data[4];
            }else{
                $data['goods']['is_rx']=false;
            }*/
            $aData['goods']['is_rx'] = $this->str_to_bool($data['goods']['is_rx']);
            $obj_ident = $obj_cart_object->add_object($arr_objects[$type], $aData, $msg, false);
            if ($aData['goods']['is_rx'] == true) {
                //如果是处方药则直接跳进购物车
                $jumpTo_url = $this->gen_url(array('app' => 'b2c', 'ctl' => 'site_cart', 'act' => 'rx'));
                $this->redirect($jumpTo_url, true);
            }else{
                $jumpTo_url = $this->gen_url(array('app' => 'b2c', 'ctl' => 'site_cart'));
                $this->redirect($jumpTo_url, true);
            }
        }
        if (!$obj_ident) {
    ...
    private function update_cart(&$msg = '', $type = 'all')
    {
        ...
        $aParams['is_rx'] = $this->str_to_bool($aParams['is_rx']);
        ...
        if ($aParams['modify_quantity'] && is_array($aParams['modify_quantity'])) {
            ...
            foreach ($aParams['modify_quantity'] as $obj_ident => $arr_object) {
                ...
                $_flag = $mCartObject->update_object($aParams['obj_type'], $obj_ident, $arr_object, $aParams['is_rx']);
    ...
    {
        $msg = '';
        $is_rx = $this->str_to_bool($_POST['is_rx']);
        $this->remove_cart_object($msg, 'all', $is_rx);
        $this->_cart_main($msg, 'all');
    }
     ...
     * @return null
     */
    private function remove_cart_object(&$msg = '', $type = 'all', $is_rx = 0)
    {
        $is_rx = $this->str_to_bool($is_rx);
        $aParams = $this->_request->get_params(true);
        $mCartObject = $this->app->model('cart_objects');
        ...
        if ($aParams[0] == 'coupon') {
            ...
            if ($aParams[0] == 'all' || empty($aParams['modify_quantity'])) {
                $obj_type = null;
                if (!$mCartObject->remove_object('', null, $msg, false, $is_rx)) {
                    // 不带入参清空所有的
                    $error_json = array(
                        'error' => $msg,
                    );
                ...
                if ($aParams['modify_quantity'] && is_array($aParams['modify_quantity'])) {
                    foreach ($aParams['modify_quantity'] as $obj_ident => $arr_object) {
                        if ($arr_object['quantity']) {
                            // 删除整个商品对象.
                            if (!$mCartObject->remove_object($aParams['obj_type'], $obj_ident, $msg, false, $is_rx)) {
                                $error_json = array(
                                    'error' => $msg,
                                );
                                if ($type == 'mini') {
    ...
    public function _check_checkout($isfastbuy = 'false', $request = 'false', $is_rx = 'false')
    {
        $isfastbuy = $this->str_to_bool($isfastbuy) ?: 'false';
        $request = $this->str_to_bool($request);
        $is_rx = $this->str_to_bool($is_rx);
        ...
        // 初始化购物车数据
        $this->_common(false, $isfastbuy, $is_rx);
        ...
        $params = array(
            'app' => 'b2c', 'ctl' => 'site_cart', 'act' => 'checkout',
            'arg0' => $isfastbuy, 'arg1' => $is_rx
        );
        $this->splash('success', $this->gen_url($params), null, $request);
     ...
     * @return void
     */
    public function checkout($isfastbuy = 0, $is_rx = 0)
    {
        if ($isfastbuy === 'false') $isfastbuy = false;
        $is_rx = $this->str_to_bool($is_rx);
        ...
        $this->checkout_result($isfastbuy, $flag);
    }
     ...
     * @return null
     */
    public function checkout_result($isfastbuy = 0, $flag = '')
    {
        //预售商品下单结算的判断
        if (!$isfastbuy) {
            $prepare = kernel::service('prepare_goods');
        ...
        if ($def_addr) {
            ...
            setcookie('purchase[addr_area]', $area[2], 0, kernel::base_url() . '/');//本文件有两处设置地方-主要用于地区优惠查询
    ...
    public function delivery_change()
    {
        ...
        setcookie('purchase[addr_area]', $_POST['area'], 0, kernel::base_url() . '/');
    ...
    {
        $is_rx = $this->str_to_bool($_POST['is_rx']);
        $this->_common(false, $_POST['isfastbuy'], $is_rx);
        ...
        setcookie('purchase[addr_area]', $sdf_order['area_id'], 0, kernel::base_url() . '/');
    ...
    private function _cart_main($msg = '', $json_type = 'all')
    {
        ...
        $this->_common(1, $aParams['is_fastbuy'], $aParams['is_rx']);
    ...
    public function _common($flag = 0, $is_fastbuy = false, $is_rx = false)
    {
        // 购物车数据信息
        $aData = $this->_request->get_params(true);
        $aData['is_fastbuy'] = $this->str_to_bool($is_fastbuy);

        $is_rx = $this->str_to_bool($is_rx);
        $this->pagedata['is_rx'] = $is_rx;
        $aData['is_rx'] = $is_rx;
        ...
        // 购物车是否为空
        $this->pagedata['is_empty'] = $this->mCart->is_empty($aCart, $is_rx);
        //ajax_html 删除单个商品是触发
        if ($this->ajax_html && $this->mCart->is_empty($aCart, $is_rx)) {
            $arr_json_data = array(
                'is_empty' => 'true',
                'number' => array(
                    'cart_number' => $this->pagedata['aCart']['_cookie']['CART_NUMBER'],
