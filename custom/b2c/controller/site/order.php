...
class b2c_ctl_site_order extends b2c_frontpage
{
    ...
    public function create()
    {
        ...
        if (isset($_POST['isfastbuy']) && $_POST['isfastbuy']) {
            $is_fastbuy = 'true';
            $cart_filter['is_fastbuy'] = $is_fastbuy;
        } else {
            $is_fastbuy = false;
            $cart_filter = array();
        }
        $is_rx = $this->str_to_bool($_POST['is_rx']);
        $cart_filter['is_rx'] = $is_rx;
        $aCart = $this->mCart->get_objects($cart_filter);
        ...
        if ($is_rx === false) {
            if (!$_POST['delivery']['shipping_id'] || !$_POST['payment']['pay_app_id']) {
        ...
        if ($result) {
            ...
            $this->cart_model->remove_object('', null, $mag, $is_fastbuy, $is_rx);
        ...
        if ($result) {
            ...
            if($is_rx===false){
                ...
                $this->end(true, $this->app->_("需求提交成功！"), $this->gen_url(array('app' => 'b2c', 'ctl' => 'site_paycenter', 'act' => 'index', 'arg0' => $order_id, 'arg1' => 'true')), '', true);
            }
