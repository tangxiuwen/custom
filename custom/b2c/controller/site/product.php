...
class b2c_ctl_site_product extends b2c_frontpage{
    ...
    public function index() {
        ...
        $productBn = $_getParams[0];

        $productId = $productsModel->getRow('product_id',array('bn' =>$productBn));
        if(empty($productId)){
        ...
        $this->pagedata['prescription'] = app::get('b2c')->getConf('system.goods.prescription');
    ...
    function _get_product_basic($productId,$aGoods,$siteMember){
        ...
        $productBasic['general_name'] = $aGoods['general_name'];//通用名称
        $productBasic['approval_number'] = $aGoods['approval_number'];//批准文号
        $productBasic['enterprise'] = $aGoods['enterprise'];//生产企业
        $productBasic['dosage_form'] = $aGoods['dosage_form'];//剂型
        $productBasic['product_qualification'] = $aGoods['product_qualification'];//说明书

        $productBasic['prescription'] = $aGoods['prescription'];
    ...
    */
    ...
    public function record_mob($data = array()){
        $data['mobile'] = $_POST['mob'];
        $data['product_id'] = $_POST['product_id'];
        $gid = $this->app->model('products')->getRow('goods_id',array('product_id'=> $_POST['product_id']));
        $data['goods_id'] = $gid['goods_id'];
        $data['create_time'] = time();
        $model = $this->app->model('consulting');
        $list = $model->getRow('id',array('product_id'=> $_POST['product_id'],'mobile'=> $_POST['mob']));
        if(empty($list)){
            $model->save($data);
        }
        exit;
