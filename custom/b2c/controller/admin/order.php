...
class b2c_ctl_admin_order extends desktop_controller{
    ...
    public function _views(){
        ...
        $sub_menu = array(
            ...
            2=>array('label'=>app::get('b2c')->_('待支付'),'optional'=>false,'filter'=>array('pay_status'=>array('0','3'),'status'=>'active','disabled'=>'false','is_rx'=>'N')),
            ...
            5=>array('label'=>app::get('b2c')->_('货到付款'),'optional'=>false,'filter'=>array('payment'=>array('-1'),'status'=>'active','disabled'=>'false','is_rx'=>'N')),
            6=>array('label'=>app::get('b2c')->_('活动'),'optional'=>false,'filter'=>array('status'=>'active','disabled'=>'false','is_pass'=>'Y')),
            ...
            16=>array('label'=>app::get('b2c')->_('待处理药方订单'),'optional'=>false,'filter'=>array('status'=>'active','disabled'=>'false','is_rx'=>'Y','ship_status'=>'0')),
            17=>array('label'=>app::get('b2c')->_('审核通过药方订单'),'optional'=>false,'filter'=>array('status'=>'active','disabled'=>'false','is_rx'=>'Y','is_pass'=>'Y','ship_status'=>'0')),
            18=>array('label'=>app::get('b2c')->_('审核不通过药方订单'),'optional'=>false,'filter'=>array('status'=>'active','disabled'=>'false','is_rx'=>'Y','is_pass'=>'N','ship_status'=>'0')),
            );
        //新留言订单
    ...
    public function docheck()
    {
        ...
        $rx_mdl = $this->app->model('rx_order');
        $get = $_GET;
        if ($get['status'] == 1) {
            $order_info = $rx_mdl->getList('order_info', array('order_id' => $get['id']));
            $order_data = unserialize($order_info[0]['order_info']);
            //审核通过调用接口
            $obj_apiv = kernel::single('b2c_apiv_exchanges_request');
            $obj_apiv->rpc_caller_request($order_data, 'ordercreate');

            $orders_mdl->update(array('is_pass' => 'Y'), array('order_id' => $get['id']));
        } elseif ($get['status'] == 0) {
            $orders_mdl->update(array('is_pass' => 'N'), array('order_id' => $get['id']));
        }

        $this->redirect("index.php?app=b2c&ctl=admin_order&act=index&view={$get['view']}&page={$get['page']}");
    }
