...
class b2c_mdl_cart_objects extends dbeav_model
{
    ...
    public function add_object($aData, $sType = 'goods')
    {
        ...
        foreach ($this->_get_cart_object_apps() as $object) {
            if ($object->get_type() == $sType) {
                $aResult = $object->add_object($aData);
                break;
            }
        }
        return $aResult;
     ...
     * @return array()
     */
    public function get_object($sType = null, $sIdent = null, $is_rx = 'false')
    {
        ...
        if (empty($sType)) {
            ...
            foreach ($this->_get_cart_object_apps() as $object) {
                $arr = $object->getAll(true, false, $is_rx);
     ...
     * @param string $msg
     */
    public function remove_object($sType = '', $sIdent = null, &$msg = '', $isfastbuy = false, $is_rx = 0)
    {
        if ($isfastbuy) {
        ...
        if ($is_rx) {
            ...
            if (empty($sIdent)) {
                unset($_SESSION['b2c_cart_objects_rx'][$member_ident]);
            } else {
                unset($_SESSION['b2c_cart_objects_rx'][$member_ident][$sIdent]);
            }
            return true;
        }
        ...
        foreach ($this->_get_cart_object_apps() as $object) {
            if (in_array($sType, $object->get_part_type())) {
                if (method_exists($object, 'remove_object_part')) {
                    $status = $object->remove_object_part($sIdent, $sType, $quantity, $msg);
                    break;
                }
            }
        }
     ...
     * @return array()
     */
    public function update_object($sType = '', $sIdent, $quantity, $is_rx = false)
    {
        if (empty($sType)) return false;
        foreach ($this->_get_cart_object_apps() as $object) {
            if ($object->get_type() == $sType) {
                $status = $object->update($sIdent, $quantity, $is_rx);
                break;
            }
        }
        //$this->setCartNum();
    ...
    private function parent_getlist($cols, $filter, $offset, $limit, $orderType)
    {
        if ($filter['is_fastbuy'] == 'true' || $this->is_fastbuy == true) {
            $arr = $_SESSION['b2c_cart_objects_fastbuy'][$filter['member_ident']];
            return $arr ? $arr : array();
        }

        if ($filter['is_rx']) {
            $arr = $_SESSION['b2c_cart_objects_rx'][$filter['member_ident']];
            $arr = $this->filter_getlist($arr, $filter);
            return $arr ?: array();
        }
    ...
    private function parent_save($data, $mustUpdate)
    {
        ...
        if ($data['is_rx'] === true) {
            if ($data['obj_type'] == 'goods') {
                $id = $data['obj_ident'];
                $_SESSION['b2c_cart_objects_rx'][$data['member_ident']][$id] = $data;
            }
            return true;
        }
