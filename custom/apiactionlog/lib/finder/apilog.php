...
class apiactionlog_finder_apilog{
    ...
    public function column_editbutton($row)
    {
        ...
        $log = $model->getList("apilog,log_type",array('apilog_id'=>$row['apilog_id']));
        if (($row['status'] == 'fail' || $row['status'] == 'sending') && $row['api_type'] == 'request'){

			if($log[0]['log_type'] == 'qy'){
				$str_operation = '<a icon="sss.ccc" target="{onComplete:function(){if (finderGroup&&finderGroup[\'' . $_GET['_finder']['finder_id'] . '\']) finderGroup[\'' . $_GET['_finder']['finder_id'] . '\'].refresh();}}" href="index.php?app=qy&ctl=admin_apilog&act=re_request&p[0]=' . $row['apilog_id'] . '" label="重试"><span><!--todo ICON-->' . app::get('b2c')->_('重试') . '</span></a>';
			}else{
